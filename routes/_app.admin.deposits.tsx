...
function AdminDeposits() {
  ...
  const doConfirm = async (id: string) => {
    try {
      const { error } = await supabase.rpc("confirm_deposit", { _request_id: id });
      if (error) throw error;
      toast.success("Confirmed"); qc.invalidateQueries({ queryKey: ["admin-deposits"] });
    }
    catch (e: any) { toast.error(e?.message ?? "Failed"); }
  };
  const doReject = async (id: string) => {
    try {
      const { error } = await supabase.rpc("reject_deposit", { _request_id: id, _reason: "Rejected by admin" });
      if (error) throw error;
      toast.success("Rejected"); qc.invalidateQueries({ queryKey: ["admin-deposits"] });
    }
    catch (e: any) { toast.error(e?.message ?? "Failed"); }
  };
          ...
          {rows.map((r: any) => (
            <li key={r.id} className="card-elevated p-4">
              <div className="flex items-center justify-between">
                <div>
                  <div className="font-semibold">{money(Number(r.amount_usd))} · {r.coin} on {r.network}</div>
                  <div className="mt-1 text-xs text-muted-foreground">User {r.user_id.slice(0, 8)} · {new Date(r.created_at).toLocaleString()}</div>
                  {r.tx_hash && <div className="mt-1 break-all text-xs text-muted-foreground">tx: {r.tx_hash}</div>}
                </div>
